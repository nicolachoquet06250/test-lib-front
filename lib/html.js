...
export const html = (selector, tpl) => {
    const component = (params = {}) => {
        ...
        c.setAttribute('component', selector);
        ...
        const observer = new MutationObserver(mutations_list => {
            mutations_list.forEach(mutation => {
                mutation.removedNodes.forEach(
                     ...
                     */
                    removedNode => {
                        const _selector = Array.from(savedComponents.entries())[Array.from(savedComponents.entries())
                            .map(([k, _]) => {
                                return removedNode.hasAttribute('component') && removedNode.getAttribute('component') === k;
                            })
                            .indexOf(true)][0]
